...
def generator(x, y, config, name):
    ...
    x = tf.layers.conv2d(x, config["nc_im"], kernel_size = config["ker_size"], padding = "same", activation = tf.nn.tanh, name = name + "/generator/tail", reuse = tf.AUTO_REUSE)
...
def conv_block(inputs, out_channels, ker_size, name):
    x = tf.layers.conv2d(inputs, out_channels, kernel_size = ker_size, padding = "same" ,activation = None, name = name + '/conv', reuse = tf.AUTO_REUSE)
